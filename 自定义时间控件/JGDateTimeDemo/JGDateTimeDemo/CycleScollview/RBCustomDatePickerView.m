...
//    NSString *weekString = [self fromDateToWeek:dateString];
    ...
    resultTimeStr=[NSString stringWithFormat:@"%@-%ld-%ld %@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)]];
...
//    NSString *weekString = [self fromDateToWeek:dateString];
   // nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d:%02d",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt,secondInt];
    ...
    resultTimeStr=[NSString stringWithFormat:@"%ld-%ld-%ld %02ld:%02ld",yearInt,monthInt,dayInt,hourInt,minuteInt];
